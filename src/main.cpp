...
void printTime(struct tm timeinfo, int px, int py) {
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            // Web Page Heading
            client.println("<body><h1>ESP32 Web Server</h1>Regras:");
            
            for (int r=0; r < 8; r++) {
              if (regraReles[r] == "") continue;
              client.println("<p>RELE " + String(r+1) + " > " + regraReles[r] + "</p>");
            }
